    ...
    : QWidget(parent)
{
    ...
    penColorButton = new QPushButton(tr("更改"));
...
void MainWindow::ShowPenStyle(int styleValue)
{
    ...
    int value = penWidthSpinBox->value();
    Qt::PenStyle style = Qt::PenStyle(
                penStyleComboBox->itemData(styleValue, Qt::UserRole).toInt());
    Qt::PenCapStyle cap = Qt::PenCapStyle(
                penCapComboBox->itemData(penCapComboBox->currentIndex(), Qt::UserRole).toInt());
    Qt::PenJoinStyle join = Qt::PenJoinStyle(
                penJoinComboBox->itemData(penJoinComboBox->currentIndex(), Qt::UserRole).toInt());
...
void MainWindow::ShowPenCap(int capValue)
{
    ...
    Qt::PenCapStyle cap = Qt::PenCapStyle(
                penCapComboBox->itemData(capValue, Qt::UserRole).toInt());
    Qt::PenJoinStyle join = Qt::PenJoinStyle(
                penJoinComboBox->itemData(penJoinComboBox->currentIndex(), Qt::UserRole).toInt());
    paintArea->SetPen(QPen(color, value, style, cap, join));
}
...
void MainWindow::ShowPenJoin(int joinValue)
{
    ...
    Qt::PenCapStyle cap = Qt::PenCapStyle(
                penCapComboBox->itemData(penCapComboBox->currentIndex(), Qt::UserRole).toInt());
    Qt::PenJoinStyle join = Qt::PenJoinStyle(
                penJoinComboBox->itemData(joinValue, Qt::UserRole).toInt());
    paintArea->SetPen(QPen(color, value, style, cap, join));
}
...
void MainWindow::ShowBrush(int value)
{
    ...
    Qt::BrushStyle style = Qt::BrushStyle(
                brushStyleComboBox->itemData(value, Qt::UserRole).toInt());
    if (style == Qt::LinearGradientPattern) {
       ...
       QConicalGradient conicalGradient(200, 200, 30);
        ...
        paintArea->SetBrush(QBrush(QPixmap("001.png")));
    } else {
